export const createDailyProducts = (data) => {
  if (!data || data.length <= 1) return document.createDocumentFragment();

  const iconSrc = data[0].icon;
  const actualProducts = data.slice(1);
  ...
  container.innerHTML = `
    <div class="w-full lg:w-[200px] flex flex-col items-center justify-center text-center gap-3.5 shrink-0">
      ...
      <h2 class="text-[22px] font-bold text-[#222] my-4 leading-tight">پرفروش ترین محصولات</h2>
      <a href="${data[0].link ?? "#"}" class="px-[18px] py-2 border border-[#222] rounded-[20px] bg-transparent text-sm cursor-pointer transition-colors duration-200 lg:hover:bg-[#222] lg:hover:text-white inline-block">
        مشاهده بیشتر
      </a>
    </div>
